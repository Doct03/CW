...
playground.src = "pics/playgroundGreen.png"
...
let food={                                       //рандом спавн еды
    ...
    let worm=[]; // создаем пустой массив данных
...
function drawlose(){                      //функция появления надписи game over ,retry
    ...
    document.addEventListener("keydown",direction); //оброботчкик событий который будет обротатывать любые запросы на странице(когда клавиша была нажата)
    ...
    function direction(event) {                          //управление червяком с помощью стрелок
        if(event.keyCode == localStorage.getItem('controlLeft') && bob != "right")     //добавлено что нельзя нажать стрелку передвежения вправо пока червяк ползет влево
            bob = "left";
        else if(event.keyCode == localStorage.getItem('controlUp') && bob != "down")   //добавлено что нельзя нажать стрелку передвежения вниз пока червяк ползет вверх
            bob = "up";
        else if(event.keyCode == localStorage.getItem('controlRight') && bob != "left")   //добавлено что нельзя нажать стрелку передвежения влево пока червяк ползет вправо
            bob = "right";
        else if(event.keyCode == localStorage.getItem('controlDown') && bob != "up")  //добавлено что нельзя нажать стрелку передвежения вверх пока червяк ползет вниз
            bob = "down";
    }
